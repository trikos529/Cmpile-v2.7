...
def ensure_environment(log_func, compiler_preference=None, reinstall_tools=False):
    ...
    if reinstall_tools:
        ...
        # Check if any tools exist before removal
        tools_exist = any(os.path.exists(d) for d in dirs_to_remove)
        
        if not tools_exist:
            log_func("No existing tools found. Installing fresh...", "bold yellow")
        else:
    ...
    # Note: If user selected one, we prioritize adding that one to PATH
...
def get_compiler_for_file(filepath, preference=None):
    """Returns the appropriate compiler executable."""
    is_cpp = not filepath.endswith(('.c', '.C'))
...
class CmpileBuilder:
    ...
    def fix_issues(self, source_files, compiler_flags=None, extra_includes=None, extra_lib_paths=None, extra_link_flags=None, extra_packages=None, build_dll=False, no_console=False, use_cmake=False, compiler_preference=None):
        ...
        self.log("Cleaning build artifacts...", "bold blue")
        try:
            # Determine project root based on the first source file
            if source_files:
                project_root = os.path.dirname(os.path.abspath(source_files[0]))
                ...
                if os.path.exists(build_dir):
                    try:
                        ...
                        self.log(f"Failed to remove build directory: {e}", "bold yellow")
                ...
                if os.path.exists(out_dir):
                    try:
                        ...
                        self.log(f"Removed output directory: {out_dir}")
                    except Exception as e:
                        self.log(f"Failed to remove output directory: {e}", "bold yellow")
                
                # Clean CMakeLists.txt if it was generated by Cmpile
                ...
                if os.path.exists(cmake_lists_path):
                    try:
                        with open(cmake_lists_path, "r") as f:
                            ...
                            if first_line == "# Generated by Cmpile":
                                os.remove(cmake_lists_path)
                                self.log(f"Removed generated CMakeLists.txt")
                    except Exception:
                        pass
        except Exception as e:
            self.log(f"Failed to clean build artifacts: {e}", "bold yellow")
        
        # 3. Auto-fix common compilation errors
        self.log("Analyzing source files for common issues...", "bold blue")
        
        # Expand source files to include all files in directories
        expanded_files = []
        ...
        for src in files:
            try:
                with open(src, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    if '#include <iostream>' in content and not '#include <iostream>' in content:
                        # This is a simplified check - in reality we'd need more sophisticated analysis
                        pass
                    
                    # Check for common vcpkg packages that might be missing
                    # This is a heuristic approach
                    if 'nlohmann/json.hpp' in content:
                        issues_found.append(('missing_package', 'nlohmann-json', src))
                    if 'fmt/format.h' in content:
                        issues_found.append(('missing_package', 'fmt', src))
                    if 'spdlog/spdlog.h' in content:
                        issues_found.append(('missing_package', 'spdlog', src))
                    if 'SDL2/SDL.h' in content:
                        issues_found.append(('missing_package', 'sdl2', src))
                    if 'raylib.h' in content:
                        issues_found.append(('missing_package', 'raylib', src))
                    if 'glm/glm.hpp' in content:
                        issues_found.append(('missing_package', 'glm', src))
                    if 'GLFW/glfw3.h' in content:
                        issues_found.append(('missing_package', 'glfw3', src))
                    if 'GL/glew.h' in content:
                        issues_found.append(('missing_package', 'glew', src))
                    if 'imgui.h' in content:
                        issues_found.append(('missing_package', 'imgui', src))
                    if 'zlib.h' in content:
                        issues_found.append(('missing_package', 'zlib', src))
                    if 'openssl/' in content:
                        issues_found.append(('missing_package', 'openssl', src))
                    if 'boost/asio.hpp' in content:
                        issues_found.append(('missing_package', 'boost-asio', src))
                    if 'QApplication' in content or 'QWidget' in content:
                        issues_found.append(('missing_package', 'qtbase', src))
            except Exception as e:
                self.log(f"Could not analyze {src}: {e}", "bold yellow")
        
        # Install missing packages
        if issues_found:
            missing_packages = set()
            for issue_type, package, src_file in issues_found:
                if issue_type == 'missing_package':
                    missing_packages.add(package)
                    self.log(f"Detected potential missing package '{package}' in {os.path.basename(src_file)}")
            
            if missing_packages:
                self.log(f"Installing missing packages: {', '.join(missing_packages)}", "bold blue")
                for pkg in missing_packages:
                    if not vcpkg_mgr.is_package_installed(pkg):
                        if not vcpkg_mgr.install_package(pkg):
                            self.log(f"Failed to install package: {pkg}", "bold red")
                        else:
                            self.log(f"Successfully installed package: {pkg}", "bold green")
                    else:
                        self.log(f"Package '{pkg}' is already installed")
        
        # 4. Rebuild with fixes applied
        self.log("Rebuilding with fixes applied...", "bold blue")
        
        # Call build_and_run with clean=True and reinstall_tools=True
        return self.build_and_run(
            ...
            extra_packages=list(missing_packages) if 'missing_packages' in locals() else extra_packages,
            build_dll=build_dll,
            no_console=no_console,
            use_cmake=use_cmake,
            compiler_preference=compiler_preference,
            reinstall_tools=True  # Reinstall tools as part of fix
        )
    ...
    def build_and_run(self, source_files, compiler_flags=None, clean=False, run=True, extra_includes=None, extra_lib_paths=None, extra_link_flags=None, extra_packages=None, build_dll=False, no_console=False, use_cmake=False, compiler_preference=None, reinstall_tools=False):
        ...
        if use_cmake:
            ...
            # Check if we should generate CMakeLists.txt
            should_generate = False
            if not os.path.exists(cmake_lists_path):
                should_generate = True
            elif clean:
                # Only overwrite if it was generated by Cmpile
                try:
                    with open(cmake_lists_path, "r") as f:
                        first_line = f.readline().strip()
                        if first_line == "# Generated by Cmpile":
                            ...
                            self.log("Existing CMakeLists.txt not generated by Cmpile. Preserving it.", "bold yellow")
                except Exception:
                    # Could not read file, safer to not overwrite
                    self.log("Could not read CMakeLists.txt. Preserving it.", "bold yellow")

            # If not exists or (clean AND generated by us), generate it
            if should_generate:
        ...
        def compile_single_file(src):
            compiler = get_compiler_for_file(src, preference=compiler_preference)
        ...
        cpp_in_use = any(not src.endswith(('.c', '.C')) for src in files)
        
        linker = None
        if compiler_preference == "llvm":
             if cpp_in_use:
                 if download_script.is_tool_on_path("clang++"): linker = "clang++"
                 elif os.path.exists(os.path.join(download_script.LLVM_DIR, "bin", "clang++.exe")): linker = os.path.join(download_script.LLVM_DIR, "bin", "clang++.exe")
             else:
                 if download_script.is_tool_on_path("clang"): linker = "clang"
                 elif os.path.exists(os.path.join(download_script.LLVM_DIR, "bin", "clang.exe")): linker = os.path.join(download_script.LLVM_DIR, "bin", "clang.exe")
        elif compiler_preference == "winlibs":
             if cpp_in_use:
                 if download_script.is_tool_on_path("g++"): linker = "g++"
                 elif os.path.exists(os.path.join(download_script.WINLIBS_DIR, "bin", "g++.exe")): linker = os.path.join(download_script.WINLIBS_DIR, "bin", "g++.exe")
             else:
                 if download_script.is_tool_on_path("gcc"): linker = "gcc"
                 elif os.path.exists(os.path.join(download_script.WINLIBS_DIR, "bin", "gcc.exe")): linker = os.path.join(download_script.WINLIBS_DIR, "bin", "gcc.exe")
        ...
        if not linker:
            ...
            if cpp_in_use:
                ...
                elif download_script.is_tool_on_path("cl"): linker = "cl"
                else: linker = GPP_EXE
            else:
                if download_script.is_tool_on_path("clang"): linker = "clang"
...
def main():
    ...
    # Handle file-independent operations
    if args.reinstall_tools:
        ui.display_status("Reinstalling tools...")
        try:
            vcpkg_mgr = ensure_environment(cli_logger, reinstall_tools=True)
            ui.display_success("Tools installation/reinstallation completed!")
        except Exception as e:
            ui.display_error(f"Failed to reinstall tools: {e}")
        return
    
    if args.fix:
        # If no files provided, just fix the environment
        if not args.files:
            ...
            # Use fix_issues method with provided files
            builder.fix_issues(
                args.files,
    ...
    if args.compiler:
        ui.display_status(f"Compiler preference set to: {args.compiler}")
        # Note: Compiler preference is stored and used when building
        # If no files provided, just set the preference
        if not args.files:
            ui.display_status("No files provided. Compiler preference will be used when building files.")
            return
    
    # Check if files are provided for compilation operations
    if not args.files:
        ui.display_error("No files provided. Please specify files to compile or use file-independent flags.")
        return
    
    # Regular build operation
